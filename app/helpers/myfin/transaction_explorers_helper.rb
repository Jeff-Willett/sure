    ...
    rows.map do |row|
      {
        ...
        entity_amounts: row.entity_amounts.transform_values { |amount| amount.to_s("F") },
        entity_labels: row.entity_labels,
        entity_id: row.entity_id,
        entity_name: row.entity_name,
        transfer: row.transfer,
        wdg: row.wdg,
        wdg_category_id: row.wdg_category_id,
        wdg_rollup: row.wdg_rollup,
        wdg_rollup_display: row.wdg_rollup || t("myfin.transaction_explorer.not_applicable"),
        wdg_rollup_id: row.wdg_rollup_id,
        jpw: row.jpw,
        jpw_category_id: row.jpw_category_id,
        detail_category: row.detail_category,
        detail_category_id: row.detail_category_id,
        detail_scheme: row.detail_scheme_name,
        scheme_id: transaction_explorer_scheme_ids[row.detail_scheme_name],
        category_update_url: myfin_entry_transaction_explorer_classification_path(row.entry_id),
        tags: row.tag_names.join(", "),
        tag_ids: row.tag_ids,
        tag_update_url: tags_transaction_path(row.entry_id),
